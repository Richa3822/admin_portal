...
function UserSellerDetails() {
        ...
        if (isEditUserMode) {
            ...
            getData(`user/${userId}`)
                .then(data => {
                    ...
                    if (data.status) {
                        setObject('initialValues', data.user)
                    }
    ...
    useEffect(() => {
        ...
        if (statesObject.selectedCountryId !== "") {
            async function fetchStates() {
                const data = await getData(`address/states/${statesObject.selectedCountryId}`)
                setStates('states', []);
    ...
    useEffect(() => {
        if (statesObject.selectedStateId !== "") {
            async function fetchStates() {
                const data = await getData(`address/cities/${statesObject.selectedStateId}`)
                setStates('cities', []);
    ...
    function setData(data, key) {
        data?.map(value => {
            setStatesObject(prevStates => {
                return { ...prevStates, [key]: [...prevStates[key], { id: value.id, label: value.name, value: value.name }] }
            })
        })
    }
    ...
    function setObject(stateKey, data) {
        Object.keys(data).map(key => {
            setStatesObject(prevStates => {
                return { ...prevStates, [stateKey]: { ...prevStates[stateKey], [key]: data[key] } };
            })
        })
    }
